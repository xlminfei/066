...
try {
 ...
 $report=[ordered]@{status=$(if($issues.Count-eq 0){'PASS'}else{'FAIL'});release_id=$ReleaseId;expected_primary_assets=$plan.assets.Count;verified=$verified.Count;issues=$issues.ToArray();checked_at=[DateTimeOffset]::UtcNow.ToString('o');assets=$verified.ToArray()}
 $report|ConvertTo-Json -Depth 6|Set-Content -LiteralPath (Join-Path $PublicationDirectory 'remote_primary_assets_verified.json') -Encoding utf8
