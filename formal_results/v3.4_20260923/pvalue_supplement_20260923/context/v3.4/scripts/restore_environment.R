#!/usr/bin/env Rscript
# Restore and verify the complete R dependency closure used by v3.2 (unchanged R package versions from v3.1).
# Supported host: Ubuntu 24.04 with the pinned R 4.6.1 runtime (or the Dockerfile).
options(timeout = 300, warn = 1, repos = c(CRAN = "https://cloud.r-project.org", RSPM = "https://packagemanager.posit.co/cran/latest"))
...
if (!identical(installed_version("renv"), renv_version)) {
  ...
  urls <- c(sprintf("https://cran.r-project.org/src/contrib/renv_%s.tar.gz", renv_version),
            sprintf("https://cloud.r-project.org/src/contrib/renv_%s.tar.gz", renv_version),
