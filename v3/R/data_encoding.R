...
validate_input_data <- function(obs, sites, expected_species = NULL,
  ...
  if (anyNA(sites$Species) || any(!nzchar(trimws(sites$Species))) || anyDuplicated(sites$Species)) {
    stop("Species in sites.csv must be unique and non-empty")
  }
  for (col in c("RecordID", "ExperimentID", "Species", "SourceID")) {
    z <- as.character(obs[[col]])
    if (anyNA(z) || any(!nzchar(trimws(z)))) stop(col, " cannot be missing")
  }
...
build_binary_counts <- function(obs, panel_species) {
  ...
  one <- data.frame(Species = obs$Species,
                    HighCount = as.integer(obs$High == 1L),
                    LowCount = as.integer(obs$High == 0L),
...
row_in_span <- function(train_x, row_x, tolerance = 1e-8) {
  a <- qr(train_x, tol = tolerance)$rank
  b <- qr(rbind(train_x, row_x), tol = tolerance)$rank
  a == b
}
...
make_design_blueprint <- function(encoded_data, response_species, model,
  ...
  if (!length(vars)) {
    ...
    seen <- character(); combinations <- rep("(intercept-only)", nrow(encoded_data))
    train_idx <- match(response_species, encoded_data$Species)
    train_matrix <- matrix(1, length(train_idx), 1L)
    estimable <- rep(TRUE, nrow(encoded_data))
    return(list(X = x, columns = character(), variables = vars, references = list(),
                levels = list(), seen = seen, combination_seen = rep(TRUE, nrow(encoded_data)),
                training_combinations = "(intercept-only)",
                raw_seen = list(),
                fixed_effect_estimable = estimable, training_species = response_species,
                train_matrix = train_matrix, rank = 1L, model = model))
  }
  ...
  for (var in vars) {
    ...
    if (is.null(ref) || !ref %in% lev) ref <- lev[[1L]]
  ...
  list(X = x, columns = colnames(x), variables = vars, references = references,
       ...
       training_combinations = unique(train_combinations),
       raw_seen = raw_seen,
       fixed_effect_estimable = estimable, training_species = response_species,
...
prepare_state <- function(input_dir, expected_species = NULL, expected_records = NULL,
  ...
  obs <- utils::read.csv(obs_path, check.names = FALSE, stringsAsFactors = FALSE,
                         na.strings = c("", "NA"))
  sites <- utils::read.csv(sites_path, check.names = FALSE, stringsAsFactors = FALSE,
                           na.strings = c("", "NA"))
  validate_input_data(obs, sites, expected_species, expected_records)
  ...
  for (model in V3_MODELS) {
    ...
    blueprints[[paste0("joint_", model)]] <- make_design_blueprint(encoded$data, joint_species, model)
  }
