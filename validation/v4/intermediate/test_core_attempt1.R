...
for(r in ROUTES)for(m in MODELS)for(d in c("full",names(DESIGNS))) {
  ...
  if(d=="full"&&m=="M3") {
    spec<-fit_spec(state,r,m,b$train_species,file.path(root,"stan/joint_bb.stan"))
    current<-b;current$request<-spec$request;current$key<-spec$key;current$version<-VERSION
