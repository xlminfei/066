...
for(route in ROUTES) {
  ...
  stopifnot(nrow(score)==16,all(is.finite(score$LogPredictiveDensityRaw)))
...
write_json_atomic(list(status="PASS",purpose="SYNTHETIC_INTERFACE_TEST_ONLY",new_synthetic_fits=2L,
  ...
  cached_reuse_checked=TRUE,stale_cache_rejected=TRUE,records_per_fit=48L,held_out_records_per_fit=16L),
