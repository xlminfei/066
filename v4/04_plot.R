...
plot_cv_metrics <- function(metrics,path) {
  ...
  for(d in names(DESIGNS))for(r in ROUTES) {
    ...
    for(metric in fields) {
      ...
      plot(seq_along(MODELS),y,xaxt="n",pch=19,col=cols,xlim=c(.35,4.65),ylim=limits,xlab="",ylab=metric,main=metric)
...
plot_hypothesis_tests <- function(tests,path) {
  ...
  for(d in names(DESIGNS)) {
    ...
    mtext(paste(d,"| BH within each panel: M1, M2, M3 (n = 3)"),side=3,outer=TRUE,line=.3)
