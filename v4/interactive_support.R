.interactive_session <- new.env(parent = emptyenv()) # 保存本交互会话实际加载的代码身份，不放入原有模型缓存。
interactive_begin <- function(root, output_dir, loaded_analysis_id, loaded_support_id) { # 将刚加载的代码绑定到本次会话。
  if (!identical(loaded_analysis_id, analysis_identity(root))) stop("加载后核心代码已变化；请重启R并重新加载。") # 防止先加载旧函数、后把新磁盘代码记为运行来源。
  if (!identical(loaded_support_id, sha256_file(file.path(root, "interactive_support.R")))) stop("交互工具文件已变化；请重新加载。") # 核对刚加载的日志和恢复工具。
  .interactive_session$root <- normalizePath(root, mustWork = TRUE) # 固定本会话所属的项目目录。
  .interactive_session$core <- loaded_analysis_id # 保存已经加载的核心代码身份。
  .interactive_session$support <- loaded_support_id # 保存已经加载的交互工具身份。
  .interactive_session$main <- sha256_file(file.path(root, "interactive_analysis.R")) # 记录本会话采用的逐行主文件。
  interactive_status(root, output_dir, "initialize", "INITIALIZED") # 只标记初始化，尚未准备或拟合。
  invisible(TRUE) # 初始化成功后继续后面的查看和准备步骤。
} # 交互会话初始化函数结束。
interactive_check_loaded <- function(root) { # 每个计算块之前确认磁盘代码仍对应本会话加载的函数。
  if (!exists("core", envir = .interactive_session, inherits = FALSE)) stop("请先执行第1段的interactive_begin。") # 禁止未初始化就进入后续计算。
  if (!identical(normalizePath(root, mustWork = TRUE), .interactive_session$root)) stop("本会话的项目目录发生变化。") # 防止混用两个版本目录。
  if (!identical(analysis_identity(root), .interactive_session$core)) stop("核心代码或设置在加载后发生变化；请重启R并使用新输出目录。") # 在旧内存函数使用新磁盘身份前停止。
  if (!identical(sha256_file(file.path(root, "interactive_support.R")), .interactive_session$support)) stop("交互工具在加载后被修改；请重新开始会话。") # 拒绝混合使用不同交互工具版本。
  if (!identical(sha256_file(file.path(root, "interactive_analysis.R")), .interactive_session$main)) stop("逐行主文件在会话中被修改；请重新开始会话。") # 最终发表运行应来自固定的同一份主文件。
  invisible(TRUE) # 所有来源身份一致才继续。
} # 已加载来源检查函数结束。
...
interactive_step <- function(step, root, output_dir, code) { # 执行一个完整R表达式或代码块，并保留成功、报错和中断记录。
  ...
  value <- tryCatch(withCallingHandlers({ interactive_check_loaded(root); eval(expression, envir = caller) }, # 在调用者环境真正执行表达式，沿用原有底层函数。
...
interactive_refresh_state <- function(root, output_dir) { # 恢复阶段开始前核对输入，并恢复磁盘参数。
  interactive_check_loaded(root) # 在恢复参数前确认核心程序和交互文件都没有在会话中被改写。
...
check_interactive_example <- function(root, output_dir, task, spec) { # 在真正拟合前核对展开的示例仍是正式计划中的同一任务。
  ...
  if (route == "binary") { # 分类公式可能包含权重表达式，因此不能只比较已保存的Stan代码字符串。
    actual_code <- as.character(brms::make_stancode(spec$formula, data = spec$data, family = brms::bernoulli(), prior = spec$prior, save_pars = brms::save_pars(all = TRUE))) # 根据控制台当前公式重新生成真正会使用的模型代码。
    actual_data <- brms::make_standata(spec$formula, data = spec$data, family = brms::bernoulli(), prior = spec$prior) # 根据当前公式求出真正的设计数据和训练权重。
    expected_data <- brms::make_standata(expected$formula, data = expected$data, family = brms::bernoulli(), prior = expected$prior) # 根据冻结设置构造应当使用的设计数据和训练权重。
    if (!identical(actual_code, expected$code) || !identical(actual_data, expected_data)) stop("分类公式或其实际训练权重被修改。") # 权重表达式、设计矩阵或模型代码变动时，必须在采样之前停止。
  } # 分类公式的语义核对结束。
