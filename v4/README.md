...
> **新增：在Ubuntu的R中逐行运行。** 打开[每行中文注释的interactive_analysis.R](interactive_analysis.R)，按[交互使用与恢复说明](INTERACTIVE_zh.md)执行。它展开一个正式CV任务，并继续调用原有函数完成其余任务；原来的四步Rscript入口保留。
