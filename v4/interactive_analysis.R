...
loaded_analysis_id <- { load_v4(root); analysis_identity(root) } # 一个完整表达式同时加载并记录核心代码身份；不会拟合。
loaded_support_id <- { source(file.path(root, "interactive_support.R"), encoding = "UTF-8"); sha256_file(file.path(root, "interactive_support.R")) } # 同时加载并记录交互工具身份，不改变统计函数。
interactive_begin(root, output_dir, loaded_analysis_id, loaded_support_id) # 绑定本会话代码来源，防止中途改文件却继续使用旧内存函数。
...
actual_versions <- vapply(names(expected_versions), function(p) if (length(find.package(p, quiet = TRUE))) as.character(utils::packageDescription(p, fields = "Version")) else NA_character_, character(1)) # 读取安装版本原文，保留连字符，避免packageVersion格式化导致误报。
...
model_packages <- c("digest", "jsonlite", "brms", "rstan", "posterior") # 列出实际计算需要加载的主要包。
stopifnot(all(vapply(model_packages, requireNamespace, logical(1), quietly = TRUE))) # 再检查主要包的命名空间确实能够加载。
...
route <- "binary" # 默认展示分类；如需定量，可在开始会话前改为joint_bb，或在控制台用route <- "joint_bb"替代执行本行。
