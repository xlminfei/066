# Frozen v3.4 input/encoding definitions; only configuration symbol names changed.
...
validate_sites_panel <- function(sites, expected_species = NULL) {
  if (!all(c("Species", SITE_COLUMNS) %in% names(sites))) {
...
build_encoding_dictionary <- function(panel, rare_min = RARE_MIN) {
...
encode_panel <- function(panel, dictionary, predictor_sites = PREDICTOR_SITES) {
  panel <- validate_sites_panel(panel)
  if (!all(SITE_COLUMNS %in% names(panel))) stop("panel is missing required site columns")
  out <- panel
  ...
  list(data = out, predictor_sites = predictor_sites,
       dictionary = dictionary, raw_sites = SITE_COLUMNS)
}
