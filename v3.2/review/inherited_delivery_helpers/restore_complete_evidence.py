#!/usr/bin/env python3
"""Download, verify and reassemble the v3.1 complete evidence archive (public assets)."""
import argparse, hashlib, json, os, pathlib, re, tempfile, urllib.request
BASE = "https://github.com/xlminfei/066/releases/download/v3.1-complete/"
MANIFEST_NAME = "v3.1-complete-evidence.parts.json"
MANIFEST_SHA = "a997592eb00494b42576bee300cbb6debfb963d3145388bd03cc96c591db1ec6"
ARCHIVE_SHA = "e32e32675897c6f81ace04d24e125f3f7c805244b787773cdcda0cb44ddd71cc"
def digest(p):
    h = hashlib.sha256()
    with open(p, "rb") as f:
...
def main():
    ...
    parser.add_argument("--directory", default="v3.1-complete-data")
    ...
    for i, part in enumerate(spec["parts"], 1):
        if part["index"] != i or not re.fullmatch(r"v3\.1-complete-evidence\.zip\.part\d{3}", part["name"]): raise ValueError("Invalid part order/name")
