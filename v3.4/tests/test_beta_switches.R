...
for(ratio in c(1,2)) {
 lo<-.2;hi<-.8
 objective<-function(loga)log(tail_gap(lo,hi,exp(loga),ratio*exp(loga))/1e-4)
 loga<-uniroot(objective,c(log(1e-10),log(1e-2)),tol=1e-11)$root
 a0<-exp(loga)
 ...
 for(delta in c(.01,.0001))for(side in c(-1,1)) {
   ...
   add(paste0("tail_b_over_a_",ratio),delta,side,a,b,lo,hi,if(side<0)"quadrature" else "tail_difference","tail_log_gap_1e-4")
 }
}
cases<-do.call(rbind,cases)
# Re-evaluate the original problematic threshold and exact archived worst case.
legacy<-read.csv(file.path(root,"review","beta_switches_before_threshold_fix","cases.csv"),stringsAsFactors=FALSE)
legacy<-legacy[legacy$Threshold=="tail_log_gap_1e-7" & legacy$RelativeOffset==1e-4,names(cases)]
legacy$Group<-paste0("legacy_",legacy$Group)
legacy$ExpectedBranch<-"quadrature_after_threshold_fix"
cases<-rbind(cases,legacy)
...
stopifnot(all(cases$ReferenceTailLogDifference[cases$Threshold=="tail_log_gap_1e-4"&cases$Side<0]<1e-4),all(cases$ReferenceTailLogDifference[cases$Threshold=="tail_log_gap_1e-4"&cases$Side>0]>1e-4))
...
z$ValueScaledError<-z$ValueAbsError/pmax(1,abs(z$Reference))
...
for(key in unique(paste(z$Group,z$RelativeOffset,sep="|"))) {
 ...
 pairs[[length(pairs)+1L]]<-data.frame(Group=p$Group[1],RelativeOffset=p$RelativeOffset[1],StanValueChange=diff(p$Stan),ReferenceValueChange=diff(p$Reference),ValueChangeResidual=valerr,ValueChangeScaledResidual=abs(valerr)/max(1,abs(p$Reference)),StanGradientChange_a=diff(p$Stan_da),ReferenceGradientChange_a=diff(p$Ref_da),GradientChangeScaledResidual_a=abs(aerr)/max(1,abs(p$Ref_da)),StanGradientChange_b=diff(p$Stan_db),ReferenceGradientChange_b=diff(p$Ref_db),GradientChangeScaledResidual_b=abs(berr)/max(1,abs(p$Ref_db)))
}
p<-do.call(rbind,pairs)
write.csv(p,file.path(out,"cross_switch_changes.csv"),row.names=FALSE)
value_pass<-all(is.finite(z$Stan))&&all(z$ValueScaledError<1e-10)
