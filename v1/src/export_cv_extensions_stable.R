...
EXTENSION_VERSION <- "cv_extensions_20260916_stable_beta_v2"
source("/project/work/ratio_analysis_20260914/scripts/stable_cv_likelihood.R")
...
analysis_root <- "/project/work/ratio_analysis_20260914"
assert(identical(cv$outcome,"joint") && identical(cv$request$type,"phylo_distance") &&
  cv$model %in% c("M1_P","Site315_P","M2_P","M3_P","Phylogeny_only_P"),"Stable exporter is restricted to the five reviewed repairs")
assert(identical(cv$likelihood_evaluation$version,STABLE_BETA_INTERVAL_VERSION) &&
  identical(cv$likelihood_evaluation$sampling_changed,FALSE),"Missing fixed-draw numerical repair identity")
helper_names <- c("stable_beta_interval.R","stable_cv_likelihood.R")
helper_hashes <- setNames(vapply(helper_names,function(n)sha(file.path(analysis_root,"scripts",n)),character(1)),helper_names)
assert(identical(unlist(cv$likelihood_evaluation$helper_sha256),helper_hashes),"Likelihood helper version differs from CV repair")
compatibility_manifest_relative <- "review/likelihood_diagnosis_20260916/stable_compatibility_sources.csv"
compatibility_manifest <- file.path(analysis_root,compatibility_manifest_relative)
compatibility_sources <- utils::read.csv(compatibility_manifest,stringsAsFactors=FALSE)
compatibility_file <- file.path(analysis_root,"review/likelihood_diagnosis_20260916/stable_compatibility_all.csv")
protected <- c(parent_path,file.path(analysis_root,"scripts",helper_names),compatibility_file,cv$likelihood_evaluation$raw_fold_files$File)
for(p in protected) {
  relative <- substring(p,nchar(analysis_root)+2L)
  expected <- compatibility_sources[compatibility_sources$File==relative,,drop=FALSE]
  assert(nrow(expected)==1L && sha(p)==expected$SHA256 && file.info(p)$size==expected$Bytes,"Compatibility evidence hash mismatch")
}
compatibility <- utils::read.csv(compatibility_file,stringsAsFactors=FALSE)
assert(nrow(compatibility)==100L && !anyDuplicated(paste(compatibility$Model,compatibility$CVType,compatibility$Fold)) &&
  all(compatibility$OriginalNonfiniteTraining==0L) && all(compatibility$CorrectedNonfinite==0L) &&
  all(compatibility$MaxAbsTrainingJointLogDifference<=1e-8),"Joint numerical compatibility gate not satisfied")
...
for(k in seq_len(fold_count)) {
  ...
  if(outcome=="binary") {
    ...
    raw_lm<-as_chain_matrix(la[,,held,drop=FALSE])
    repaired<-stable_cv_loglik(ff,source,held,parent$species)
    lm<-repaired$log_lik
    assert(identical(dim(lm),dim(cv$evaluated_log_lik[[k]])) &&
      max(abs(lm-cv$evaluated_log_lik[[k]]))<1e-10,"Stable source likelihood differs from saved fixed-draw score")
    assert(repaired$diagnostic$OriginalNonfiniteTraining==0L &&
      repaired$diagnostic$MaxAbsTrainingJointLogDifference<=1e-8,"Training likelihood numerical compatibility failed")
    ...
    raw_identity <- cv$likelihood_evaluation$raw_fold_files[cv$likelihood_evaluation$raw_fold_files$Fold==k,,drop=FALSE]
    assert(nrow(raw_identity)==1L && raw_identity$File==cache_path && raw_identity$SHA256==sha(cache_path) &&
      raw_identity$DataKey==cache$data_key,"Original fold file changed after score repair")
...
manifest$NumericalLikelihoodMethod <- STABLE_BETA_INTERVAL_VERSION
manifest$LikelihoodHelperSHA256 <- as.list(helper_hashes)
manifest$ScoringUnchangedModel <- TRUE
manifest$RawFoldFiles <- cv$likelihood_evaluation$raw_fold_files
manifest$OriginalNonfiniteHeldout <- sum(vapply(cv$likelihood_evaluation$fold_diagnostics,function(x)x$OriginalNonfiniteHeldout,numeric(1)))
manifest$CompatibilityManifest <- compatibility_manifest_relative
manifest$CompatibilityManifestSHA256 <- sha(compatibility_manifest)
