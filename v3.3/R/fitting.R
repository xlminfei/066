binary_template_key_v32 <- function(spec) sha256_object(list(stan_code=spec$code, model=spec$request$model, columns=spec$blueprint$columns))
...
fit_from_spec_v3 <- function(spec,out_path,force=FALSE) {
  ...
  if(!force && cache_is_valid(out_path,req,req$route,req$model,req$train_weighting)) {
    ...
    if(req$route=="binary") { if(!exists(".v31_binary_templates",.GlobalEnv,inherits=FALSE))assign(".v31_binary_templates",new.env(),.GlobalEnv);assign(binary_template_key_v32(spec),b$fit,get(".v31_binary_templates",.GlobalEnv)) }
    return(b)
  }
  ctl<-req$sampling
  ...
  if(req$route=="binary") {
    ...
    templates<-get(".v31_binary_templates",.GlobalEnv);codekey<-binary_template_key_v32(spec)
