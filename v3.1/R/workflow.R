...
run_stage_v3 <- function(root,stage,args=list()) {
  ...
  if(stage%in%c("cv","all")) {
    ...
    write_stage_receipt_v3(root,state,"cv",file.path("results",c("cv_record_predictions.csv","cv_fold_metrics.csv","cv_metrics_summary.csv","model_vs_null.csv","training_method_comparisons.csv","calibration_bins.csv","roc_coordinates.csv")))
  }
