...
write_results_report_v3 <- function(root) {
  res<-file.path(root,"results");required<-c("cv_metrics_summary.csv","model_vs_null.csv","training_method_comparisons.csv","training_ppc_summary.csv")
  ...
  lines<-c("# v3.1 computed analysis report","",paste("Version:",V3_VERSION),"",
  ...
  writeLines(lines,file.path(root,"reports","REPORT_v3_1.md"),useBytes=TRUE)
}
