resolve_v3_root <- function(args=commandArgs(TRUE),script=NULL) {
  ...
  if(!file.exists(file.path(candidate,"R","config.R")))stop("Invalid v3.1 root: ",candidate)
  candidate
}
load_v3_modules <- function(root,envir=.GlobalEnv,configure=TRUE) {
  for(f in c("config","data_encoding","weights","metrics","applicability","cache_io","fitting","prediction","cross_validation","comparison","plotting","ppc","provenance","reporting","audit_run","workflow"))source(file.path(root,"R",paste0(f,".R")),local=envir)
