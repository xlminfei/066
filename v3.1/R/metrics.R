...
evaluate_evidence <- function(evidence, eval_weighting = "record_equal", route = NULL) {
  ...
  out<-data.frame(Route=route,EvalWeighting=eval_weighting,RecordsUsed=nrow(z),SpeciesUsed=length(unique(z$Species)),WeightSum=sum(wf$weight),
    ...
    RMSE=if(any(point))weighted_rmse(z$ObservedPoint[point],z$PredictedPoint[point],wp) else NA_real_,PointRecords=sum(point),
