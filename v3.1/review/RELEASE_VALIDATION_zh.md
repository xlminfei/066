...
- 首次发布仅在Git目录保存数值快照，原始fit.rds当时留在本地。本轮按用户要求补传完整证据附件：全部现存fit.rds、smoke、历史快照、说明和中间文件均纳入 working-v3/ 命名空间，逐项校验而不按大小省略。review/compiled_fit_inventory.csv的原说明描述首次发布策略，当前完整收录情况以 docs/COMPLETE_ARCHIVE_zh.md 和完整归档清单为准。
...
本轮补充：旧等值图的固定值公式已单独解释；新差异化夹具和醒目TEST标题通过T12。逐函数修改说明细化为C01—C44，并补充C45归档/C46上传；所有通过声明均关联实际证据，完整正式研究运行仍未开始。
