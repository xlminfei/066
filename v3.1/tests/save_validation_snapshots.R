...
for(p in files) {
  ...
  inventory[[key]]<-data.frame(Path=key,Bytes=file.info(p)$size,SHA256=sha256_file(p),FitKey=b$key,FitPayloadHash=b$fit_payload_hash,
    Included="posterior/data/diagnostics retained in validation_snapshots.rds; compiled native fit cache stays local")
}
...
cat("VALIDATION_SNAPSHOT_PASS",length(files),"synthetic fits; no native compiled binaries in package\n")
